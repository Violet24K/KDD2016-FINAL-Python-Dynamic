...
if __name__ == '__main__':
    ...
    graph_file = open('datasets/' + DATASET_NAME + '-temporal/node_attr_change.txt', 'r')
    all_line = graph_file.readlines()
    graph_file.close()
    num_line = len(all_line)
    ...
    acc_record = []
    time_record = []
    for line_index in range(num_line):
        items = all_line[line_index].strip().split(',')
        change_node_in_sub = int(items[0])
        change_from = node_attr_matrix_2[change_node_in_sub, change_node_in_sub]
        change_to = int(items[1])
        counter += 1
        node_attr_matrix_2[change_node_in_sub, change_node_in_sub] = change_to
        node_attr_matrix_1[match_from_sub_to_whole(change_node_in_sub), match_from_sub_to_whole(change_node_in_sub)] = change_to
        node_attr_matrix_acce_2[change_node_in_sub, change_from] = 0
        node_attr_matrix_acce_2[change_node_in_sub, change_to] = 1
        node_attr_matrix_acce_1[match_from_sub_to_whole(change_node_in_sub), change_from] = 0
        node_attr_matrix_acce_1[match_from_sub_to_whole(change_node_in_sub), change_to] = 1
