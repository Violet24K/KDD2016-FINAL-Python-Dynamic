...
if __name__ == '__main__':
    ...
    for line_index in range(start_line, end_line):
        ...
        edge_attr = int(items[2])
        t += 1
        adj_matrix_1[node1, node2] = 1
        adj_matrix_1[node2, node1] = 1
        edge_attr_matrix_1[node1, node2] = edge_attr
        edge_attr_matrix_1[node2, node1] = edge_attr

        if is_in_subgraph(node1) and is_in_subgraph(node2):
            if edge_attr_matrix_2[match_from_whole_to_sub(node1), match_from_whole_to_sub(node2)] != edge_attr:
                sub_change = 1
        if (sub_change == 0):
            continue
        ...
        adj_matrix_2[node_sub_1, node_sub_2] = 1
        adj_matrix_2[node_sub_2, node_sub_1] = 1
        edge_attr_matrix_2[node_sub_1, node_sub_2] = edge_attr
        edge_attr_matrix_2[node_sub_2, node_sub_1] = edge_attr
